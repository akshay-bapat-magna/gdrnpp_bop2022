...
OUTPUT_DIR = "output/gdrn/doorlatch/convnext_a6_AugCosyAAEGray_BG05_mlL1_DMask_amodalClipBox_classAware_doorlatch"
INPUT = dict(
    DZI_PAD_SCALE=1.5,
    TRUNCATE_FG=True,
    CHANGE_BG_PROB=0.0,
...
SOLVER = dict(
    ...
    ANNEAL_POINT=0.1,
    OPTIMIZER_CFG=dict(_delete_=True, type="Ranger", betas=(0.888, 0.999), lr=1e-6, weight_decay=0.01),
    MOMENTUM=0.888,
    LR=1e-6,
    WEIGHT_DECAY=0.0,
    WARMUP_FACTOR=0.001,
...
VAL = dict(
    ...
    USE_SEG=False,   # Whether to use segmented masks along with detections
)
