...
class DOORLATCH_BOP_TEST_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/doorlatch/test_pbr
    ...
    def __call__(self):
        ...
        for scene_id, int_im_id in tqdm(scene_im_ids):
            ...
            for anno_i, anno in enumerate(gt_dict[str_im_id]):
                ...
                if self.name == "doorlatch_bop_test_pbr":
...
SPLITS_DOORLATCH = dict(
    doorlatch_bop_test_pbr=dict(
        name="doorlatch_bop_test_pbr",
    ...
    doorlatch_bop_test_pbr_alignK=dict(
        name="doorlatch_bop_test_pbr_alignK",
...
for obj in ref.doorlatch.objects:
    name = "doorlatch_{}_bop_test_pbr".format(obj)
    select_objs = [obj]
    if name not in SPLITS_DOORLATCH:
        SPLITS_DOORLATCH[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/doorlatch/test_pbr"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/doorlatch/models_cad"),
            objs=[obj],
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_DOORLATCH:
        used_cfg = SPLITS_DOORLATCH[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, DOORLATCH_BOP_TEST_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_DOORLATCH.keys())
